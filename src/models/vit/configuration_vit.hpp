...
public:
    ...
    explicit ViTConfig(const string &model_type = "base", int patch_ = 32, int hw = 112, int classes = 1000, string act_fn_type_ = "GELU", const string &name_type = "vit") {
        names_config.init(name_type);
        class_size = classes;
        patch = patch_;
        img_hw = hw;
        if (model_type == "base") {
            hidden_dim = 384;    // Width
            head_size = 6;       // Number of heads
            ffn_hidden = 1536;   // MLP
            block_num = 12;    
        } else {
